...
int analysis::InitRun(PHCompositeNode *topNode)
{
  ...
  tree->Branch("tjet_cent", &m_tjet_centrality);
  tree->Branch("rjet_cent", &m_rjet_centrality);
...
int analysis::process_event(PHCompositeNode *topNode)
{
  ...
  for (JetMap::Iter iter = tjets->begin(); iter != tjets->end(); ++iter)
  {
    ...
    assert(tjet);
    m_tjet_centrality =  cent_node->get_centile(CentralityInfo::PROP::bimp);
    ...
    for (JetMap::Iter riter = rjets->begin(); riter != rjets->end(); ++riter)
      {
	m_rjet_centrality =  cent_node->get_centile(CentralityInfo::PROP::epd_NS);
